...
from scripts.handle_path import DATA_DIR
...
class HandleExcel(object):
    ...
    def __init__(self, sheet_name, filename=None):
        if filename is None:
            self.filename = os.path.join(DATA_DIR, c_yaml.read("excel", "cases_path"))
        else:
    ...
    def write_response(self, row, res_text):
        self.write_data(row=row,
                        column=c_yaml.read("excel", "actual_col"),
                        value=res_text)

    def write_success_result(self, row):
        self.write_data(row=row,
                        column=c_yaml.read("excel", "result_col"),
                        value=c_yaml.read('msg', 'success_result'))

    def write_fail_result(self, row):
        self.write_data(row=row,
                        column=c_yaml.read("excel", "result_col"),
                        value=c_yaml.read('msg', 'fail_result'))

if __name__ == '__main__':
    read = HandleExcel('connect_cases.xlsx', 'group')
